...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    $stmt = $db->prepare("SELECT * FROM Ability1;");
    $stmtErr =  $stmt -> execute();
    $abilities = $stmt->fetchAll();
    foreach ($abilities as $ability1) {
        setcookie($ability1['a_name'].'_value', '', 100000);
    }
   
    if (isset($_POST['powers'])) {
        foreach ($_POST['powers'] as $item) {
            foreach ($abilities as $ability1) {
                if ($ability1['a_name'] == $item) {
            ...
            foreach ($_POST['powers'] as $item) {
                foreach ($abilities as $ability1) {
                    if ($ability1['a_name'] == $item) {
                        ...
                        $stmtErr = $stmt->execute(['p_id' => $_SESSION['uid'], 'a_id' => $ability1['a_id']]);
                        break;
                    }
                }
            ...
            if (isset($_POST['powers'])) {
                foreach ($_POST['powers'] as $item) {
                    foreach ($abilities as $ability1) {
                        if ($ability1['a_name'] == $item) {
                            $stmt = $db->prepare("INSERT INTO Person_Ability (p_id, a_id) VALUES (:p_id, :a_id);");
                            $stmtErr = $stmt->execute(['p_id' => intval($strId), 'a_id' => $ability1['a_id']]);
                            break;
                        }
                    }
                    if (!$stmtErr) {
